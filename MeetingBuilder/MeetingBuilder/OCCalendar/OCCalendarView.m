...
- (void) dateSelected
{
    [self.calendarViewDelegate removeCalendarFromView];
}
...
-(BOOL) clickedOnArrow:(UITouch *)touch
{
...
-(void) renderCalendarViewForPhone
{
    ...
    for(int i = 0; i < dayTitles.count; i++) {
        ...
        [((NSString *)[dayTitles objectAtIndex:i]) drawInRect: dayHeaderFrame withAttributes:attributes];
        CGContextRestoreGState(context);
    }
    ...
    NSDictionary *attributes = @{ NSFontAttributeName: font,
                                  NSParagraphStyleAttributeName: paragraphStyle };
    [monthTitle drawInRect: textFrame withAttributes:attributes];
    CGContextRestoreGState(context);
...
-(void) renderCalendarViewForPad
{
    ...
    //// Color Declarations
    UIColor* bigBoxInnerShadowColor = [UIColor whiteColor];
    UIColor* backgroundLightColor = [UIColor whiteColor];
    UIColor* lineLightColor = [UIColor whiteColor];
    UIColor* lightColor = [UIColor whiteColor];
    UIColor* darkColor = [UIColor whiteColor];
    UIColor* boxStroke = [UIColor blackColor];
