...
@implementation CustomSubtitledCell
...
- (IBAction)onClickDelete {
    ...
    [self.subTitledDelegate deleteCell:self.deleteButton.tag];
}
...
-(void)animateSlidingButton:(BOOL)moveRight
{
    ...
    if (moveRight)
    {
        self.deleteButton.frame = CGRectMake(oldFrame.origin.x+DELETE_BUTTON_WIDTH, oldFrame.origin.y, oldFrame.size.width, oldFrame.size.height);
    }
    else
    {
        self.deleteButton.frame = CGRectMake(oldFrame.origin.x-DELETE_BUTTON_WIDTH, oldFrame.origin.y, oldFrame.size.width, oldFrame.size.height);
