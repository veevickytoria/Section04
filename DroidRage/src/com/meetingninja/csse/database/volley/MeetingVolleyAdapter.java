...
public class MeetingVolleyAdapter extends MeetingDatabaseAdapter {
	...
	public static void fetchMeetingInfo(final String meetingID,final AsyncResponse<Meeting> delegate) {
		...
		JsonNodeRequest req = new JsonNodeRequest(_url, null,
			...
			public void onResponse(JsonNode response, int statusCode,
				...
				if (response != null) {
					...
					VolleyLog.e("Error:%n %s",error.getLocalizedMessage());
				}
...
//		String url = MeetingDatabaseAdapter.getBaseUri().appendPath(meetingID).build().toString();
//		StringRequest dr = new StringRequest(Request.Method.DELETE, url,new Response.Listener<String>() {
//			@Override
