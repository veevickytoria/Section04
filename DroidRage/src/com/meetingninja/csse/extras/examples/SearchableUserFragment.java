...
public class SearchableUserFragment extends Fragment implements
	...
	private Bundle saveState() {
		...
		ArrayList<UserParcel> save_allUsers = new ArrayList<UserParcel>();
		for (User user : allUsers) {
			save_allUsers.add(new UserParcel(user));
		}
		state.putParcelableArrayList(Keys.User.LIST + "ALL", save_allUsers);

		ArrayList<UserParcel> save_addedUsers = new ArrayList<UserParcel>();
		for (User user : addedUsers) {
			save_addedUsers.add(new UserParcel(user));
		}
		state.putParcelableArrayList(Keys.User.LIST + "ADDED", save_addedUsers);
	...
	public void onTokenAdded(Object arg0) {
		...
		if (added != null) {
			...
			if (!NinjaTextUtils.isEmpty(added.getID())) {
	...
	public void onTokenRemoved(Object arg0) {
		...
		if (removed != null) {
			...
			if (!NinjaTextUtils.isEmpty(removed.getID())) {
