...
public class UnderlinePageIndicatorPicker extends View implements PageIndicator {
	...
	public UnderlinePageIndicatorPicker(Context context, AttributeSet attrs,
		...
		mColorUnderline = getResources().getColor(
				R.color.dialog_text_color_holo_dark);

		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.BetterPickersDialogFragment, defStyle, 0);
		mColorUnderline = a
				.getColor(
						R.styleable.BetterPickersDialogFragment_bpKeyboardIndicatorColor,
						mColorUnderline);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mTitleView != null) {
			...
			if (mPositionOffset > 0f && mCurrentPage < count - 1) {
				...
				lineLeft = (mPositionOffset * nextTabLeft + (1f - mPositionOffset)
						* lineLeft);
				lineRight = (mPositionOffset * nextTabRight + (1f - mPositionOffset)
						* lineRight);
			}

			canvas.drawRect(lineLeft, getPaddingBottom(), lineRight,
